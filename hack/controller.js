...
function wrapClass(Controller) {
  ...
  function methodToMiddleware(Controller, key) {
    function classControllerMiddleware(...args) {
      ...
      // # MODIFIED #
      // ############
      const result = utils.callFn(controller[key], args, controller);
      if (result instanceof Promise) {
        return result.then(data => {
          if (data !== undefined) {
            controller.ctx.body = data;
          }
          return data;
        });
      }
      controller.ctx.body = result;
      return result;
    }
    // ############
    // # MODIFIED #
    // ############
    classControllerMiddleware.Controller = Controller;
    classControllerMiddleware.key = key;
